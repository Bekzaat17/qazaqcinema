...
export interface Auth {
  ...
  // Недельный бесплатный выбор. `weekly_pick_available` — выбор этой недели свободен
  // (показываем приглашение вместо пэйволла); `weekly_movie_id` — фильм ТЕКУЩЕЙ недели
  // (бейдж «Менің таңдауым», платить не зовём); `week_ends_at` — когда окно закроется,
  // один на всех (окно общее: понедельник 00:00 по Алматы).
  weekly_pick_available: boolean;
  weekly_movie_id: number | null;
  week_ends_at: string | null;
  // Фильм, подаренный ПРОШЛОЙ, одноразовой механикой: бесплатен навсегда. У новых
  // пользователей всегда null — забрать такой подарок больше нельзя.
  free_view_movie_id: number | null;
  // @-имя публичного канала без «@»: шторка гейта ведёт в него. Приходит с бэка, а не из
  // VITE-переменной, чтобы вторая копия имени не разъехалась ссылкой в никуда.
...
export const api = {
  ...
  /** Триггер защищённой выдачи: бот пришлёт видео в личку (protect_content).
   *
   * `useWeeklyPick` — согласие потратить недельный выбор. Без него сервер его не тратит:
   * иначе неделя сгорала бы от случайного перехода по ссылке на фильм.
   * `gift` в ответе — видео ушло бесплатно (недельный выбор либо старый подарок).
   *
   * 403 `no_access` — пэйволл; 403 `need_channel` — не подписан на канал (это НЕ стена,
   * до бесплатного фильма остался один шаг, см. `ChannelGateSheet`).
   */
  play: (id: number, useWeeklyPick = false) =>
    request<{ status: "sent"; gift: boolean; daily: boolean }>(
      `/api/movies/${id}/play${useWeeklyPick ? "?use_weekly_pick=true" : ""}`,
      { method: "POST" },
    ),
