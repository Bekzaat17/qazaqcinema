...
## 📍 ТЕКУЩАЯ ПОЗИЦИЯ
**⏭️ ЗАПЛАНИРОВАНО (следующее) — Фаза 13: браузинг каталога по категориям + бесконечные карусели.
НЕ НАЧАТО.** Полные шаги и продуманный UX — в разделе «## Фаза 13» ниже. Кратко: (1) отдельная
страница-каталог с мультивыбором категорий (чипы-чекбоксы, пустые категории скрыты) и пагинацией
(grid постеров 3-в-ряд, подгрузка при скролле); (2) точки входа с главной — кнопка «Каталог» +
«Барлығы →» на каждой полке (открывает каталог, преднастроенный на категорию); (3) на главной каждая
карусель — последние **14** фильмов с эффектом бесконечной прокрутки. **Вся выборка/ограничение — на
бэке:** `/home` отдаёт готовые полки по 14 последних (а не весь каталог — критично при 1000+ фильмах),
плюс пагинация каталога (`GET /api/movies` + `categories`/`page`/`limit`/`total`).
...
## Фаза 13 — Каталог: браузинг по категориям + бесконечные карусели (⛔ НЕ НАЧАТО)

> Цель: дать пользователю листать весь каталог по категориям (мультивыбор) с пагинацией; а на
> главной — компактные полки из последних **14** фильмов с бесконечной прокруткой. Сейчас главная =
> hero + полки (полное содержимое категории), отдельной страницы-каталога нет, поиск — отдельной
> строкой. Пустые категории уже не показываются на главной (полки строятся из фильмов, порог ряда ≥3,
> `web/src/lib/catalog.ts`) — тот же принцип переносим на фильтр каталога.
>
> **Ключевой принцип — вся выборка/ограничение НА БЭКЕ (не на фронте):** и каталог (пагинация), и
> главная (последние 14 на полку) ограничивает сервер. Сейчас `/home` отдаёт ВЕСЬ каталог, а полки
> режет фронт — при 1000+ фильмах это мегабайты на каждый вход. Клиент должен получать ровно то, что
> рисует.

**UX/UI (продумано):**
- **Точки входа с главной:** (а) кнопка-иконка «Каталог» (`LayoutGrid` из lucide) в топбаре ИЛИ
  отдельной строкой-кнопкой под поиском; (б) на каждой полке заголовок + «Барлығы →» справа →
  открывает каталог, преднастроенный на эту категорию (Netflix-style «показать всё»).
- **Страница-каталог** — полноэкранный оверлей, как существующие шторки, со своей нативной
  `BackButton` (встроить в стек оверлеев `App.tsx`: `anyOverlay`/`onBack`):
  - Сверху — **липкий ряд чипов-категорий с чекбоксами** (мультивыбор, горизонтальный скролл).
    «Барлығы» сбрасывает фильтр. Показываем ТОЛЬКО непустые категории (пустые не отображаются).
  - Тело — **grid постеров 3-в-ряд** (как результаты поиска, `PosterCard inShelf={false}`),
    сортировка по новизне (id desc).
  - **Пагинация — подгрузка при прокрутке** («Тағы жүктеу» / сентинел `IntersectionObserver`),
    мобайл-френдли; альтернатива — нумерованные страницы (решить в 13.0).
  - Состояния: скелетон страницы, пустой результат под фильтром, ошибка+ретрай.
- **Главная — полки:** каждая категория-полка = **последние 14** (newest, **лимит на бэке** — сервер
  не отдаёт весь каталог) с **эффектом бесконечной карусели** (луп при горизонтальном скролле);
  «Жаңа түскен» тоже последние 14.

### 13.0 — Бэкенд: пагинация каталога + главная только с последними 14 (⛔)
- [ ] **Главная собирается и ограничивается НА БЭКЕ (критично при росте каталога).**
      `GET /api/movies/home` сейчас отдаёт ВЕСЬ каталог одним ответом (`{hero, movies}`), а полки
      режет фронт (`buildShelves`) — при 1000 фильмах это мегабайты и рост на каждый вход. Должно
      стать: сервер собирает **готовые полки** `{hero, shelves: [{key, title, movies}]}`, где каждая
      полка — **последние 14** (newest) своей категории, порог ряда ≥3 сохраняется, «Жаңа түскен» —
      последние 14 всего. **Размер ответа = O(категории × 14), НЕ O(каталог)** — фронт получает ровно
      столько, сколько рисует.
- [ ] Репозиторий: «последние N на категорию» эффективным запросом — window-function
      `ROW_NUMBER() OVER (PARTITION BY category ORDER BY id DESC) <= 14` (одним SQL; либо per-category
      `LIMIT 14` — категорий немного) + счётчики категорий для порога ≥3/непустых. Композиция полок
      (сейчас на фронте `web/src/lib/catalog.ts buildShelves`) переезжает в `CatalogService`.
- [ ] Схема ответа меняется: `CatalogHomeOut {hero, shelves: [ShelfOut{key, title, movies: [MovieOut]}]}`
      (вместо `{hero, movies}`). Кэш `catalog:main` EX 600 + инвалидация в `ingest` — как есть (Фаза
      11.2); ответ теперь маленький и стабильный по размеру.
- [ ] Порт `MovieRepository` (`application/ports/repositories.py`): метод пагинации, напр.
      `list_page(categories: list[str], limit: int, offset: int) -> tuple[list[Movie], int]`.
      Мультикатегория = `WHERE category = ANY(:cats)` (пустой список → без фильтра), `ORDER BY id DESC`,
      `LIMIT/OFFSET`; второй элемент — `COUNT(*)` тем же фильтром (для `has_more`/номеров страниц).
- [ ] (Опц., для чипов-чекбоксов) `category_counts() -> dict[str, int]` (`GROUP BY category`) —
      фронт покажет только непустые категории (+ счётчики на чипах). Либо вывести из первой страницы
      каталога/из `/home` — решить (лишний запрос vs точность).
- [ ] `CatalogService.browse(categories, page, limit)` → `(list[Movie], total)` (клампы внутри).
- [ ] `GET /api/movies` расширить: `?categories=a,b&page=1&limit=24` → новая схема
      `MoviePageOut {items: list[MovieOut], total, page, limit, has_more}`. Либо отдельный
      `GET /api/movies/browse` — решить. ⚠️ Специфичные пути объявлять ДО `/{movie_id}` (как
      `hero`/`home`), иначе матчится как id.
- [ ] Кэш: браузинг агрегированно НЕ кэшируем (много комбинаций фильтр×страница) — прямой БД
      (rate-limit роутера `/api/movies` уже покрывает: 100/10с на IP). `/home` остаётся cache-aside.
- [ ] Клампы: `limit ≤ 48`, `page ≥ 1`; страж `telegram_file_id` не течёт в `MoviePageOut`.

### 13.1 — Фронт: страница-каталог (чекбоксы категорий + пагинация) (⛔)
- [ ] API-клиент (`lib/api.ts`): `api.browse(categories, page, limit)` → `MoviePageOut` (типизировать);
      гонки страниц гасить монотонным `reqId` (как в дебаунс-поиске `App.tsx`).
- [ ] Компонент каталога: липкий ряд чипов-чекбоксов (мультивыбор, из непустых категорий; лейблы —
      `categoryLabel`), grid постеров, подгрузка при скролле (сентинел; состояние `page/items/hasMore/
      loading`, сброс `items` при смене фильтра).
- [ ] Точки входа: кнопка «Каталог» (топбар/под поиском) + «Барлығы →» на заголовках полок
      (`Shelf.tsx` — заголовок кликабелен, открывает каталог с преднастроенной категорией). Встроить
      в стек оверлеев + `BackButton`.
- [ ] DEV-мок (`lib/devMock.ts`): отдавать пагинированный `/api/movies?categories&page&limit`
      (срез + total) — чтобы каталог работал в браузере без бэка.

### 13.2 — Фронт: главная — рендер полок с бэка + бесконечная карусель (⛔)
- [ ] `api.home()` → новый `CatalogHomeOut {hero, shelves}`; `App.tsx` рендерит `shelves` напрямую.
      Кап-14 и композицию полок на клиенте (`buildShelves`) **УБРАТЬ** — лимит и сборка теперь на бэке
      (13.0), фронт только отображает. DEV-мок (`lib/devMock.ts`) обновить под новую форму `/home`.
- [ ] Бесконечная карусель полки — **решить реализацию:** (а) JS-луп (клон элементов + сброс
      `scrollLeft` на границах) — даёт ручной бесшовный луп, НО конфликтует с принципом «карусели —
      нативный CSS, без JS-каруселей» (CLAUDE.md) → обосновать исключение; либо (б) CSS-marquee
      (автоскролл, нативно) — но это не ручной луп. В обоих случаях уважать `prefers-reduced-motion`
      (нет движения → обычный scroll-snap), не ломать `no-scrollbar`/snap.

### Тесты / DoD (⛔)
- [ ] Бэк: репозиторий-пагинация (страница/оффсет/total/мультикатегория) — интеграционный (нужен
      postgres); `CatalogService.browse` (клампы/пустой фильтр) — юнит на фейке; страж — нет
      `telegram_file_id` в `MoviePageOut`/`CatalogHomeOut`.
- [ ] Бэк: главная ограничена — при >14 фильмах в категории полка отдаёт РОВНО 14 (newest), размер
      ответа `/home` не растёт с каталогом (интеграционный на «последние N на категорию»).
- [ ] Зелёное: `ruff` + `mypy` + `pytest` в контейнере; `tsc -b` + `vite build`; браузер-превью
      (каталог: фильтр/подгрузка/точки входа; главная: 14 + луп).

**Открытые решения (флаг при старте фазы):** (1) пагинация — подгрузка-при-скролле vs нумерованные
страницы; (2) бесконечная карусель — JS-луп vs CSS-marquee (хотелка эффекта против принципа «без
JS-каруселей»); (3) `/api/movies` расширить vs отдельный `/api/movies/browse`; (4) счётчики на чипах
(нужен `category_counts` — доп. эндпоинт или вывести из данных).
