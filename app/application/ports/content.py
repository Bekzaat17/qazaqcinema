...
class ContentRepository(Protocol):
    async def next_for(self, source: Source, today: date) -> ContentItem | None:
        ...
        3. затем группа (`source.vary` — рубрика либо автор), которая уже остыла:
           выходившая меньше `VARIETY_COOLDOWN` назад уступает остальным. Мягко — ключом
           сортировки: остыли все, значит решает LRU, и пост всё равно выйдет;
        4. внутри — никогда не постившиеся (`last_posted_at IS NULL`), потом самые давние (LRU);
        5. `source.repeat = False` → уже постившиеся не берём вовсе: пул исчерпан → None,
           и слот переходит к следующему источнику (см. `plan.Slot.sources`).
        Тай-брейк — хэш `slug`: устойчив между прогонами, но не повторяет порядок заливки
        (`id` выдавал в ленту весь YAML-файл пачкой). У `Vary.NONE` тай-брейк остаётся `id` —
        там порядок и есть смысл.
        """
        ...
