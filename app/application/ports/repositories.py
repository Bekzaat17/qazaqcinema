...
class UserRepository(Protocol):
    ...
    async def claim_weekly_pick(self, telegram_id: int, movie_id: int, week: date) -> bool:
        """Забрать недельный бесплатный выбор; True — забрали именно мы.

        `week` — ключ окна (понедельник, см. `domain/subscription/weekly`), а не срок:
        «прошлая неделя» и «не выбирал ни разу» одинаково означают «свободно», и обе
        ситуации обязано ловить ОДНО условие запроса.

        Атомарность — как у `claim_free_view` и по той же причине: два тапа по «Көру» на
        плохой связи иначе раздали бы два разных фильма на одну неделю.
        """
        ...

    async def release_weekly_pick(self, telegram_id: int, movie_id: int, week: date) -> None:
        """Вернуть выбор, если видео не доставлено (чат с ботом не открыт).

        Сверять И фильм, И неделю: слепой сброс стёр бы выбор, сделанный уже после
        неудачной отправки, — и человек получил бы на этой неделе два фильма.
        """
        ...
