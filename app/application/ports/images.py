...
#
# Формат WebP, а не JPEG: на живом каталоге это минус четверть байт при том же
# визуальном качестве (49 → 36 КБ на копию), а сеткой из десятков карточек ограничена вся
# страница целиком — и SSR-каталог, и полки Mini App. Фолбэк на JPEG не нужен: WebP
# понимают все живые браузеры и WebView Telegram. ⚠️ Смена формата меняет и ИМЯ файла
# (`thumb_url` берёт расширение отсюда), поэтому после правки обязателен прогон
# `./start.sh thumbs --force`.
