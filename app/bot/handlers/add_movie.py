...
Картинка у фильма ровно ОДНА — постер. Ни «показывать на главной?»,
...
Сериалы: шаг «сериал» — единственное настоящее ветвление визарда,
и оно намеренно НЕ разветвляет сами хендлеры/шаги (принцип «условных шагов нет») —
это внутренний под-диалог ОДНОГО шага (кнопки → кнопки → текст),
...
# (см. модуль-докстринг про _SEASON_SKIP — это НЕ условный шаг: у серии готового сезона
# эти поля физически не существуют).
_ORDER: tuple[State, ...] = (
    AddMovie.video,
    AddMovie.category,
    AddMovie.series,
