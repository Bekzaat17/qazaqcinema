...
def render_daily_movie(movie: Movie, *, weekly_pick_renewed: bool = False) -> str:
    ...
    `weekly_pick_renewed` — понедельник: к подвалу добавляется строка о том, что окно
    недельного выбора открылось заново. ОТДЕЛЬНОГО поста про это нет намеренно: в
    понедельник в канале и так три публикации (фильм дня, квиз, разбор), а четвёртая про
    то же приложение перегрузила бы день. Пост про кино с кнопкой внутрь — ровно то место,
    где такой призыв уместен.
    """
    footer = "⏳ Тек бүгін — жазылымсыз, тегін көріңіз."
    if weekly_pick_renewed:
        footer += "\n🎟 Апталық таңдау жаңарды — арнаға жазылсаңыз, бір фильм тегін."
    return _body(movie, "📅 <b>Күн фильмі — бүгін тегін!</b>", footer=footer)
