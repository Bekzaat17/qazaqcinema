...
class BotConfig(BaseSettings):
    ...
    # Группа обсуждений, привязанная к публичному каналу (комментарии под постами).
    # Бот в ней админ с правом удалять сообщения: читает ответы на жұмбақ и убирает их
    # до разбора. 0 → хендлер комментариев инертен (dev/тесты — та же логика, что у канала).
