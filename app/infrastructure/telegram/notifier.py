...
class AiogramNotifier:
    ...
    async def notify_admins(self, text: str) -> None:
        # Каждому админу — НЕЗАВИСИМО: 403 от одного (не нажал /start, заблокировал бота)
        # не имеет права лишить уведомления остальных. Наверх молчим, только если не дошло
        # ни до кого — тогда вызывающий вправе сказать это юзеру.
        delivered = 0
    ...
    async def send_protected_video(
        ...
        Разбираем ВСЕ исходы, а не только отказ получателя: иначе `RetryAfter`
        (флуд-лимит), сеть и 5xx Telegram уходят наверх необработанными, FastAPI отдаёт
        500, и человек, нажавший «Көру», видит общее «қате шықты», не понимая, придёт
        видео или нет. Для главного действия продукта это дорого, особенно на всплеске
        одновременных запросов после поста в канале.

        Флуд-лимит пережидаем и повторяем (как `delete_message` ниже): названную самим
        Telegram паузу — обычно секунды — ждать дешевле, чем гонять человека нажимать
        заново. Не помогло → `TRY_LATER` наверх, а не 500.
        """
    ...
    async def delete_message(self, chat_id: int, message_id: int) -> DeleteOutcome:
        ...
        Исход возвращаем наверх, а причину пишем в лог: глушить отказ `suppress`
        нельзя — главный из них, «сообщение старше 48 часов», выглядел бы как успех, и
        чистка выданных видео молча не работала бы.
        ...
        Классификация — здесь, потому что это единственный слой, знающий aiogram:
          ...
          • Network / 5xx → FAILED. Не поймать их — уронить весь джоб чистки.
        """
        for attempt in (1, 2):
            try:
