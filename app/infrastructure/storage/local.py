...
class LocalPosterStorage:
    ...
    async def save(self, data: bytes, *, thumb: bytes) -> str:
        # Крупная копия всегда JPEG (`ImageProcessor` по спеке `POSTER`) → расширение
        # фиксировано, в имя не подставляем внешних строк (никакой path-инъекции);
        # uuid — без перечислимости. Расширение МЕЛКОЙ копии другое (её формат — решение
        # `POSTER_THUMB`), и подставляет его `thumb_url`, а не этот метод.
