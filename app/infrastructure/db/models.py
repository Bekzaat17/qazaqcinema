...
class UserModel(Base):
    ...
    # Недельный бесплатный выбор (`domain/subscription/weekly`). Храним КЛЮЧ ОКНА — дату
    # понедельника, а не срок: окно общее для всех, право освобождается сменой ключа, и
    # гасить его не нужно ни джобом, ни UPDATE'ом. NULL = не выбирал ни разу.
    weekly_week: Mapped[date | None] = mapped_column(Date, nullable=True)
    # Какой фильм выбран на эту неделю. FK нет намеренно — по той же причине, что у
    # `free_view_movie_id`: удаление фильма из каталога не должно ронять строку юзера.
    weekly_movie_id: Mapped[int | None] = mapped_column(nullable=True)
