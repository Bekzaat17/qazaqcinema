...
`X-Forwarded-For` (первый в списке), в dev — прямой `request.client`. Пер-юзерный
ключ (из сессии) появится в Фазе 11.1; пока IP — осознанный фолбэк «до auth».
⚠️ Мобильные юзеры Telegram часто за одним CGNAT-IP → лимиты держим щедрыми, чтобы
не ловить ложные 429 (точную пер-юзер справедливость даст сессия).
