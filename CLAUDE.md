...
- Google не видит Mini App, поэтому бэкенд отдаёт настоящий SSR: `/m/<id>-<slug>`, `/catalog`,
  `/catalog/<slug>` (см. хабы ниже), `/sitemap.xml` (приоритет 1.0 у `/catalog`, корень —
  пустая SPA), `/robots.txt`. Caddy проксирует эти пути на api до SPA-фолбэка. Рендер из
  БД на лету.
- **Страницы-хабы** — раздел (`/catalog/kids`), подборка (`/catalog/new`, `/catalog/popular`)
  и год (`/catalog/2024`) — это ОДИН обработчик и один шаблон: различаются они только
  текстами и правилом отбора, а те приходят данными из `domain/seo/hubs` (новая подборка =
  строка в `COLLECTIONS`, и она сразу получает страницу, пагинацию, врезку и строку в
  sitemap). Разбор «что живёт по слагу» существует ровно в одном месте — `resolve_hub`.
  ⚠️ Слаги подборок делят пространство имён с категориями, непересечение закреплено тестом.
  Год получает страницу от `YEAR_MIN_MOVIES`=4 фильмов: тонкая страница не ранжируется, а
  бюджет обхода тратит. Все хабы живут под `/catalog/`, чтобы не плодить пути в Caddy.
- Вид страниц собран один раз в `templates/base.html` (дизайн-система + шапка с поиском +
  подвал), страницы её наследуют и переопределяют только мета и содержимое; повторяющиеся
  куски — `_filters`, `_pager`, `_shelves`, `_card`. ⚠️ Ни одна интерактивная часть НЕ
  зависит от JS: фильтры — ссылки (у каждого состояния свой постоянный URL, иначе его не
  проиндексировать), длинные ряды чипов и полки прокручиваются CSS-ом. Тёмная тема одна:
  страница продолжает Mini App, светлой версии у продукта нет.
- Фильтры — двумя компактными рядами, как в Mini App (`CatalogView`): чипы разделов и
  строка «Сұрыптау». Года в фильтрах НЕТ: двадцать один вариант в такой ряд не влезает ни
  на одном телефоне, а страницы годов остались — на них ведёт год на карточке фильма и
  sitemap. Сортировка — это подборки (`/catalog/new`, `/catalog/popular`), а не `?sort=`:
  у подборки свой постоянный URL и свой текст, а параметр сортировки наплодил бы URL с
  одним и тем же набором фильмов в разном порядке, то есть дубли.
- Листалка по центру, номер страницы подписью под ней; на телефоне номера скрыты CSS-ом
  (остаются в разметке для обхода) — двадцать одна страница в 360 px не влезает, а
  «назад — где я — вперёд» влезает.
- Полки «Жаңа түскен»/«Танымал» — только на КАРТОЧКЕ ФИЛЬМА (`_nav(..., shelves=True)`):
  хаб сам является списком, и «новинки» под ним — второй список на той же странице.
  Карточке фильма, наоборот, нужен выход дальше. Каталог на ней — ссылками, без карточек
  (у неё своя задача, второй каталог внутри лишь утяжелит).
- Блок «вопрос — ответ» на хабах + разметка `FAQPage`. ⚠️ Расширенного сниппета она почти
  наверняка не даст (с 2023 Google показывает FAQ-результаты фактически только
  государственным и медицинским сайтам) — блок нужен ради собственного текста посадочной и
  ответа человеку, который не понял, что смотреть надо в Telegram. Разметка обязана
  повторять ВИДИМЫЙ текст: источник один — `seo.faq`.
- `SearchAction` в разметке WebSite ведёт на `/catalog?q=` — Google может показать поле
  поиска по сайту в сниппете. Разметка без живой страницы результатов не работает, поэтому
  поиск и сделан серверным.
- **Хабы листаются, а не растут**: страница отдаёт `SEO_PAGE_SIZE`=28 карточек, дальше
  ...
  пагинации перечислены в sitemap, `?page=0` → 404. Листалка — обычные `<a href>`: только
  по ним краулер доходит до карточек со второй страницы.
- **Ни одна страница не читает каталог целиком.** Свой срез — `list_page` с `LIMIT`
  ...
  `?page=3` между обходами; подборка «популярное» — осознанное исключение, её смысл ровно в
  порядке), счётчики разделов и годов — по одному `GROUP BY`, похожие — `list_related`
  (число общих категорий считает SQL). Счётчики годов запрашиваются только когда слаг похож
  на год. Единственное исключение — sitemap: он обязан перечислить каждый URL.
