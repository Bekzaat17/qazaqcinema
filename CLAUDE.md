...
- Картинка у фильма одна — постер 2:3 (`ImageProcessor`, Pillow, `ImageOps.fit`), в двух
  ...
  полок. Превью в WebP — вдвое меньше байт при том же виде, а сетками ограничены и
  SSR-страницы, и Mini App. ⚠️ URL превью нигде не хранится, его выводят `thumb_url`
