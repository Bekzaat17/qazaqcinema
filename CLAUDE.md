...
- Любое ЛИЧНОЕ сообщение, которое не разобрал никто выше, ловит `handlers/fallback` — он
  подключается ПОСЛЕДНИМ в `bot/setup.py` и отвечает шаблоном на казахском: обе дороги ведут в
  профиль Mini App (👤 в правом верхнем углу) — оттуда «Жазылу» → «Kaspi арқылы төлеу» →
  «Чекті жүктеу» и оттуда же «Қолдау қызметіне жазу», плюс кнопка 🍿. Повод не косметический:
  чек, присланный в чат с ботом, заявкой НЕ становится (`payment_requests` заводит только
  Mini App), админ его в модерации не видит, а человек уверен, что оплатил, — и раньше бот на
  такое сообщение молчал буквально, хендлера под фото в личке не было вовсе.
- ⚠️ Шаг «откройте профиль» в этом шаблоне стоит ПЕРЕД «Жазылу» не для красоты: обе кнопки
  живут только в `ProfileSheet`, на витрине их нет, и без этого шага человек их просто не найдёт.
  Подписи в тексте — РОВНО те, что он видит на экране: переименовали кнопку в Mini App —
  поправьте и шаблон (есть тест на порядок и на подписи).
- Границы фолбэка: `F.chat.type == PRIVATE` (в группе обсуждений свои хендлеры, см.
  `handlers/quiz`), `StateFilter(None)` (иначе шаблон перехватывал бы шаги визарда `/add` и
  рассылки) и `should_answer` — молчим на КОМАНДЫ (свои разобраны выше, на чужую инструкция
  про чек не ответ) и на сообщения АДМИНОВ (им в личку падают карточки чеков и обращений).
...
### Недельный бесплатный выбор (в работе)
- Приходит на смену одноразовому подарку: один фильм бесплатно на ОБЩЕЕ для всех окно —
  понедельник 00:00 по Алматы (`domain/subscription/weekly`: `week_start` — ключ окна,
  `week_end` — единая точка отсчёта счётчика для фронта). Зона та же, что у фильма дня и
  сетки канала: границы суток, недели и контент-плана обязаны совпадать, иначе «жаңа апта»
  наступит в приложении не тогда, когда о ней напишет канал.
- ⚠️ Храним КЛЮЧ ОКНА (`users.weekly_week` — дата понедельника), а не срок. Персональные
  «7 дней с момента выбора» потребовали бы колонки со временем и джоба, который эти сроки
  гасит; общее окно освобождает право сменой ключа — без единой записи в БД. NULL и
  «прошлая неделя» одинаково значат «свободно», поэтому захват сверяется через
  `IS DISTINCT FROM` (обычное `<>` дало бы NULL, и первый выбор не прошёл бы никогда).
- Цена общего окна названа честно: взявший фильм в воскресенье вечером получит вечер, а не
  неделю. Лечится напоминанием в субботу тем, кто выбор ещё не потратил, а не кодом.
- `claim_weekly_pick` / `release_weekly_pick` — атомарные, как `claim_free_view`, и по той
  же причине (два тапа по «Көру» иначе раздали бы два фильма). `weekly_*`, как и
  `free_view_*`, НИКОГДА не попадают в `upsert`.
- Одноразовый подарок (`free_view_*`) остаётся у тех, кто его уже потратил: их фильм
  бесплатен навсегда, отнимать нечего. Новым он больше не выдаётся, и признак «старый» —
  сам `free_view_movie_id IS NOT NULL`; отдельного флага не нужно.
- Гейт — подписка на публичный канал: порт `ChannelMembership` (`getChatMember`, бот там
  админ), `public_channel_id = 0` → всегда «подписан» (dev и тесты не гейтим, как и с
  публикацией). ⚠️ **Fail-open**: сбой Telegram или потерянная админка → отвечаем
  «подписан». Пустить чужого раз в неделю дешевле, чем упереть подписчика в стену и
  отправить его в поддержку; всплеск таких выдач виден в дневном отчёте.
- ⚠️ Кэш подписки (`CachedChannelMembership`, Redis, TTL 5 мин) держит ТОЛЬКО «подписан».
  Отказ не кэшируется намеренно: человек видит «жазылыңыз», идёт в канал, возвращается и
  жмёт «Тексеру» — закэшированный отказ он увидел бы снова, уже будучи подписчиком.
- Порядок оснований в `PlaybackService._resolve_gift` (единственное место правила доступа):
  подписка → фильм дня → свой недельный фильм → старый подарок → гейт канала → захват
  недельного → пэйволл. Читается как «самое дешёвое для нас первым»: бесплатное отдаётся
  раньше, чем что-либо расходуется, и у подписчика выбор НЕ тратится. Подписка на канал
  спрашивается ПОСЛЕДНЕЙ из проверок — это сетевой вызов, и ради человека, уже потратившего
  неделю, его дёргать незачем.
- ⚠️ Откат захвата (`_rollback`) — одна точка на все три ветки срыва (фильма нет, чат
  закрыт, Telegram занят). Пока он был скопирован по месту, новая ветка норовила его
  забыть — и человек терял неделю, не увидев фильма. Откатывается ТОЛЬКО свежий захват.
- `use_weekly_pick` — обязательное ЯВНОЕ согласие (как раньше у подарка): иначе выбор
  сгорал бы от deep-link `?startapp=m_42`, где человек и не думал его тратить.
- Неподписчик, который хочет взять фильм, получает `NEED_CHANNEL` (403 `need_channel`), а
  НЕ пэйволл: продавать человеку то, что он может получить бесплатно в одно действие, —
  верный способ не получить ни подписки, ни денег. Это же и событие `channel_gate`.
- В `AuthOut` `weekly_movie_id` отдаётся ТОЛЬКО пока он про текущую неделю: в колонке
  лежит и позапрошлый выбор, а бейдж на фильме с истёкшим правом обещал бы доступ,
  которого нет. Флага «подарок ещё цел» в DTO больше нет — забрать подарок нельзя.
  `week_ends_at` — один на всех (окно общее), фронт по нему рисует счётчик «3 күн қалды»
  и не считает срок по каждому. `channel_username` тоже приходит отсюда, а не из
  VITE-переменной: вторая копия имени однажды разъедется — молча, ссылкой в никуда ровно
  у тех, кого мы просим подписаться.
- ⚠️ У ПОДПИСЧИКА этой механики нет НИГДЕ: `App` обнуляет `weeklyPickAvailable`,
  `weeklyMovieId` и `weekEndsAt` при `has_access`, поэтому ни бейджа на постере, ни строки
  в профиле, ни подсказок в карточке у него не появляется. Правило одно и живёт в одном
  месте — не размазывать его по компонентам.
- Бейдж «Таңдауым» на постере берётся из контекста `useWeeklyPick`, а не из пропсов: постер
  лежит в глубине четырёх деревьев (полки, каталог, поиск, избранное) — ровно та же причина,
  по которой контекстом сделано избранное.
- Шторки: `WeeklyPickSheet` — подтверждение выбора (обязательное: неделя тратится, и там же
  названа дата «до»), `ChannelGateSheet` — гейт канала с «Арнаға өту» и «Тексеру». Второй
  открывается ПО ОТВЕТУ СЕРВЕРА (403 `need_channel`), а не по предварительной проверке:
  так гейт появляется ровно в момент, когда человек уже решил смотреть, и это же даёт
  честный счётчик `channel_gate`.
- ⚠️ Формулировки везде «қарай аласыз» / «апта соңына дейін», а НЕ «фильм жіберілді
  навсегда»: видео из чата уборка сносит через ~40 ч, а право живёт до понедельника —
  обещать файл значило бы, что на третий день человек увидит «дали и отняли».
- Напоминаний РОВНО ДВА в неделю: `weekly_pick_open` (понедельник 11:00) и
  `weekly_pick_closing` (суббота 11:00). Не 10:00 — в этот час в канал уходит фильм дня.
  Джоба, который «гасит» истёкшие выборы, НЕТ и не нужно: право освобождается сменой
  ключа недели.
- ⚠️ Аудитория писем узкая намеренно (`list_weekly_pickers` / `list_weekly_idle`): только
  те, кто механикой уже пользовался (`weekly_week IS NOT NULL`). Рассылка всей базе
  первым же письмом научила бы людей выключать уведомления. Отсекаются подписчики,
  отключившие рассылки и те, у кого не открыт чат с ботом — общие условия собраны в
  `_reminder_conditions`, чтобы два запроса не разъехались.
- Субботнее письмо — не вежливость, а лечение единственной слабости общего окна: взявший
  фильм в воскресенье вечером получает вечер, а не неделю.
- ⚠️ У субботнего письма есть ГОРИЗОНТ (`weekly.IDLE_REMINDER_WEEKS`, 4 недели), и он
  обязателен: без нижней границы «успейте взять» капало бы каждую субботу ВЕЧНО всякому,
  кто однажды выбрал фильм и ушёл. Условие `weekly_week >= since_week` заодно заменяет
  `IS NOT NULL`.
- Отдельного поста про открытие окна в канале НЕТ: в понедельник там и так три публикации
  (фильм дня, квиз, разбор). Вместо него строка в подвале поста про фильм дня
  (`render_daily_movie(weekly_pick_renewed=True)`), а «понедельник ли» считается тем же
  `week_start`, что и сам выбор.
- Отчёты: в снимок `daily_reports` добавлены `channel_gates` / `weekly_picks` /
  `weekly_plays` / `channel_joins` / `channel_leaves` (счётчики, `server_default 0` — старые
  снимки читаются как «этого тогда не было») и `channel_members` (`getChatMemberCount`). ⚠️ Последняя НУЛЛУЕМАЯ намеренно:
  ноль подписчиков и «Telegram не ответил» — разные вещи, и вторая обязана выглядеть в
  отчёте как пропуск строки, а не как обвал аудитории до нуля.
- ⚠️ Прирост канала сам по себе не доказывает ничего — он мог случиться и без нас. Поэтому
  рядом всегда стоит атрибуция `channel_gates → weekly_picks`: сколько людей мы САМИ
  привели к требованию подписки и сколько из них дошло до фильма. Дельта за сутки
  считается в тексте из вчерашнего снимка (`AnalyticsService.previous_report`), а не
  хранится: производная, как и проценты.
- Строка `🎁 Сыйлық фильм` переименована в «(ескі)» и в дневном, и в недельном отчёте: это
  наследство одноразовой механики, и она должна стремиться к нулю. Живой бесплатный крючок
  теперь `weekly_*`.

### Подписка и оплата
- `SubscriptionService.activate/revoke/expire_due/has_active_access` — единственная точка
  гранта, отзыва и проверки; способы оплаты только зовут `activate`. Доступ считается по
  `expires_at > now` в реальном времени, джоб `expire_due` (15 мин) лишь проставляет статус,
  шлёт DM и забирает видео.
- ⚠️ **Чек Kaspi открывает доступ СРАЗУ**, не дожидаясь модератора: админы не круглосуточны,
  и ночной чек иначе лежал бы до утра — человек заплатил и смотрит на пэйволл. Модерация
  после этого работает как отзыв: ✅ только помечает заявку (повторный `activate` продлил бы
  подписку на второй срок), ❌ зовёт `revoke`. `revoke` отнимает РОВНО срок этого тарифа
  (`domain/subscription/expiry.shorten`), а не обнуляет `expires_at`: чек мог продлевать живую
  подписку, и отказ не имеет права съесть оплаченный раньше остаток.
- Признак «доступ уже открыт» — `payment_requests.granted_at`. По нему ветвятся обе кнопки
  модерации, подписи кнопок и строка в карточке админа: ❌ под работающей подпиской её
  ОТКЛЮЧАЕТ, и админ обязан видеть это до нажатия.
- Исключение — `PaymentService.REJECT_LIMIT` (3) отклонённых чеков в истории человека: его
  заявка идёт прежним путём (`PENDING_REVIEW`, решает админ), и ему об этом прямо пишут.
  Без правила отказ не значил бы ничего — тот же скриншот заливается снова. Отсюда же
  `pending_review` во фронте: ветка жива, но редкая.
- Забытая заявка — не «человек ждёт», а неснятый вопрос к оплате: джоб
  `pending_proofs_reminder` (ежечасно, :05) пишет админам о `PENDING` возрастом 12–13 ч.
  Окно ОДНОГО прогона, а не «старше 12 ч»: иначе одна заявка капала бы в чат каждый час.
- `subscribe` пишется в момент выдачи, то есть авансом, до проверки чека; парное событие
  `revoke` — при отклонении. В дневном отчёте `subscribes` их не разделяет (строки под
  отзывы там пока нет).
...
- Действия админов в статистику не попадают: декоратор `AdminBlindEventRepository` на записи;
  счётчики людей исключают админов явно (`exclude`). Движения в канале это НЕ касается: админ
  канала — такой же подписчик, и его уход такой же минус, как чужой.
- Подписки и отписки канала считаются ПО ГОЛОВАМ: хендлер `bot/handlers/channel_members` ловит
  апдейты `chat_member` (бот админ канала, `allowed_updates` aiogram выводит сам из
  зарегистрированных хендлеров) и пишет их в `channel_member_events` — своя таблица, потому что
  `user_events.user_id` это внешний ключ на `users`, а на канал подписываются и те, кто бота ни
  разу не открывал. ⚠️ Апдейты приходят по всем чатам, где бот админ (ещё и группа обсуждений),
  поэтому чужие отсекаются по `public_channel_id`. В отчёте строка канала — `📣 Арна: 1512
  (+40 / −35)`: `getChatMemberCount` даёт только ИТОГ, а он скрывает отток — «пришли 40, ушли
  35» и «движения не было» это один и тот же «+5». Оба нуля → строка падает на прежнюю разность
  снимков (`(+5 тәулікте)`): за сутки до появления журнала людей просто не считали, и рисовать
  там «+0 / −0» было бы враньём.
...
- ⚠️ Ротация ещё и РАЗНООБРАЗИТ, и это не украшение: пул залит из YAML пачками (в `atau.yaml`
  подряд девять вопросов про жылқы, в `sayings.yaml` — двадцать цитат Абая), а тай-брейк по `id`
  выдавал в канал ровно порядок файла — месяц лошадей и квартал Абая при 12 рубриках и 13 авторах
  в пуле. У источника есть ось `Source.vary` (`plan.Vary`): `TOPIC` — не повторять рубрику,
  `SOURCE` — автора (нақыл сөз: рубрика одна, имён тринадцать), `NONE` — порядок и есть смысл
  (қара сөз идут по номерам, и только у них тай-брейк остался `id`). В SQL это оконный
  `max(last_posted_at)` по группе: остывшая (`VARIETY_COOLDOWN`, 3 недели = 3 выпуска) идёт
  первой, горячие — по давности, дальше LRU по элементу и тай-брейк хэшем `slug`. Правило
  мягкое — ключ сортировки, а не фильтр: у жұмбақ рубрика одна на весь пул, остывать некому,
  и слот всё равно постит — просто перемешанным, а не порядком файла. Сортировать YAML руками
  бесполезно: следующая заливка снова придёт пачкой.
