...
- Спрос — `search_queries` (append-only журнал поисков, включая нулевые результаты) — очередь на
  озвучку. В дневном отчёте это блок «Іздеп, таппағаны»: сколько искали, сколько зря и ТОП-10
  нулевых с числом попыток и числом РАЗНЫХ людей (десять попыток одного и десять человек — разный
  сигнал). Блок в снимок НЕ пишется и колонок не требует: журнал append-only, повторный запрос
  даёт то же самое (`SearchSummary`, живой запрос тем же окном `day_window`). Запрос —
  пользовательский текст, и отчёт уходит HTML → `render_report` его экранирует.
