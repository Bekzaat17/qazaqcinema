...
- **Без открытого чата с ботом кинотеатр не работает — и фронт знает это заранее**
  ...
  `t.me/<bot>?start=web` (`openTelegramLink`, а не `openLink` — иначе t.me уедет в браузер) ДО
  подарка и пэйволла. Возврат из чата ловит существующий `visibilitychange` → статус освежается сам.
...
- **Hero главной = ФИЛЬМ ДНЯ, бесплатный для всех** (решение 2026-08-19, заменяет «hero курируется»
  ...
  только форматирует: правило суток живёт в одном месте.
- **Hero рисуется в ДВУХ вариантах, широкий баннер необязателен** (решение 2026-08-19): в ротацию
  входит весь каталог, а `hero_image_url` есть далеко не у всех (было 55 из 158). Постер 2:3 нельзя
  ни растянуть, ни обрезать до 3:2 — кадр скомпонован вертикально, центр-кроп режет лицо и
  название. Поэтому `web/src/components/Hero.tsx` содержит две вёрстки одного смысла: **с баннером**
  — кинематографичный кадр 3:2 с текстом на нижнем скриме; **без баннера** — «афиша»: фоном идёт
  ТОТ ЖЕ постер, увеличенный (`scale`, обязателен — иначе blur даёт полосы по краям) и размытый,
  а поверх лежит его чёткая копия 2:3 с кольцом и тенью. Подложка берёт цвета самого фильма →
  блок каждый день выглядит нарисованным под сегодняшнее кино, а нового ассета не нужно ни одного.
  Контраст текста гарантируют скримы поверх фона, а не удача с картинкой. Пульс (`anim-pulse-ring`)
  — только на CTA и только когда кино реально бесплатно; «дыхание» фона — `anim-breathe`; обе
