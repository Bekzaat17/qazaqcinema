...
- Отчёты: в снимок `daily_reports` добавлены `channel_gates` / `weekly_picks` /
  `weekly_plays` (счётчики, `server_default 0` — старые снимки читаются как «этого тогда
  не было») и `channel_members` (`getChatMemberCount`). ⚠️ Последняя НУЛЛУЕМАЯ намеренно:
