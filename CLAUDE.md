...
- Любое ЛИЧНОЕ сообщение, которое не разобрал никто выше, ловит `handlers/fallback` — он
  подключается ПОСЛЕДНИМ в `bot/setup.py` и отвечает шаблоном на казахском: чек грузится через
  Mini App («Жазылу» → «Kaspi арқылы төлеу» → «Чекті жүктеу»), админу пишут оттуда же
  (👤 профиль → «Қолдау қызметіне жазу»), плюс кнопка 🍿. Повод не косметический: чек, присланный
  в чат с ботом, заявкой НЕ становится (`payment_requests` заводит только Mini App), админ его в
  модерации не видит, а человек уверен, что оплатил, — и раньше бот на такое сообщение молчал
  буквально, хендлера под фото в личке не было вовсе. Границы роутера — `F.chat.type == PRIVATE`
  (в группе обсуждений свои хендлеры, см. `handlers/quiz`) и `StateFilter(None)` (иначе шаблон
  перехватывал бы шаги визарда `/add` и рассылки). Подписи в тексте — РОВНО те, что человек
  видит в Mini App: меняются кнопки — меняется и шаблон.
