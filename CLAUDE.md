...
- Спрос — `search_queries` (append-only журнал поисков, включая нулевые результаты) — очередь на
  озвучку. В отчёт не входит: читается запросом, когда нужно.
...
- ⚠️ Ораза айт и Құрбан айт — по объявлению ДУМК (muftyat.kz), таблица `Lunar` заполнена по
  2030 год и её надо пополнять: года нет в таблице → поздравления не будет (промолчать
  честнее, чем поздравить не в тот день). Скорбных дат (31 мамыр) в справочнике нет
