...
- Фильтры (порядок, разделы, годы) — ОДНОЙ панелью под заголовком, активный чип подсвечен
  и ссылкой не является. Листалка по центру, номер страницы подписью под ней; на телефоне
  номера скрыты CSS-ом (остаются в разметке для обхода) — двадцать одна страница в 360 px
  не влезает, а «назад — где я — вперёд» влезает.
- Перелинковка сверх крошек одинакова на ВСЕХ страницах, включая вторые страницы пагинации
  и карточки фильмов (`_nav` в роутере): человек, пришедший из поиска на `?page=3`, попадает
  туда же, куда с первой. Полки «Жаңа түскен»/«Танымал» стоят ниже сгиба и их постеры
  `loading="lazy"` — вес первого экрана они не увеличивают. На карточке фильма каталог —
  только ссылками, без карточек (у неё своя задача, второй каталог внутри лишь утяжелит).
