...
- **Визард `/add` — навигация данными, а не «только вперёд»** (решение 2026-08-14): опечатка в
  ...
  новый шаг = +1 строка в `_ORDER`/`_PROMPTS`/`EDIT_FIELDS`, а не новый хендлер. Ветка hero
  пропускается у не-featured в ОБЕ стороны (`_seek`), правка «Жоқ→Иә» сначала добирает баннер.
